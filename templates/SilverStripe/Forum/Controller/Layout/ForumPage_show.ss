		...
		<td class="page-numbers">
			<% if $Posts.MoreThanOnePage %>
				<% if $Posts.NotFirstPage %>
					<a class="prev" href="$Posts.PrevLink" title="<%t Forum_show_ss.PREVTITLE "View the previous page" %>"><%t Forum_show_ss.PREVLINK "Prev" %></a>
				<% end_if %>
			<% end_if %>
		</td>
		<td class="gotoButtonTop" >
			<a href="#Header" title="<% _t('Forum_show_ss.CLICKGOTOTOP','Click here to go the top of this post') %>"><%t Forum_show_ss.GOTOTOP "Go to Top" %></a>
		</td>
		<td class="replyButton">
			<% if $ForumThread.canCreate %>
				<a href="$ReplyLink" title="<% _t('Forum_show_ss.CLICKREPLY', 'Click to Reply') %>"><%t Forum_show_ss.REPLY "Reply" %></a>
			<% end_if %>

			<% if $Posts.MoreThanOnePage %>
				<% if Posts.NotLastPage %>
					<a class="next" href="$Posts.NextLink" title="<%t Forum_show_ss.NEXTTITLE "View the next page" %>"><%t Forum_show_ss.NEXTLINK "Next" %> &gt;</a>
				<% end_if %>
			<% end_if %>
		</td>
	</tr>
